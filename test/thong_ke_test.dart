...
void main() {
  test('cuoiKy thang 28-31, 6 thang, nam', () {
    expect(Ngay.cuoiKy(DateTime(2026, 2, 1), 1), DateTime(2026, 2, 28));
    expect(Ngay.cuoiKy(DateTime(2026, 9, 1), 1), DateTime(2026, 9, 30));
    expect(Ngay.cuoiKy(DateTime(2026, 1, 1), 1), DateTime(2026, 1, 31));
    expect(Ngay.cuoiKy(DateTime(2026, 4, 1), 2), DateTime(2026, 9, 30));
    expect(Ngay.cuoiKy(DateTime(2026, 1, 1), 3), DateTime(2026, 12, 31));
    expect(Ngay.cuoiKy(DateTime(2026, 8, 24), 0), DateTime(2026, 8, 30));
  });

  test('Hôm nay = kỳ chứa hôm nay', () {
    final hom = DateTime(2026, 8, 30);
    expect(Ngay.dauKyHomNay(hom, 0), DateTime(2026, 8, 24));
    expect(Ngay.dauKyHomNay(hom, 1), DateTime(2026, 8, 1));
    expect(Ngay.dauKyHomNay(hom, 2), DateTime(2026, 3, 1));
    expect(Ngay.dauKyHomNay(hom, 3), DateTime(2026, 1, 1));
  });
  ...
  test('bangKy: TB nạp chỉ ngày có log, Δ cân, không bịa', () async {
    ...
    final ky = kho.bangKy(0, DateTime(2026, 8, 24));
    ...
    final trong = kho.diemCanKy(0, DateTime(2026, 8, 24));
    expect(trong.length, 2);
    expect(kho.diemCanKy(0, DateTime(2026, 8, 10)).isEmpty, isTrue);
  });
}
